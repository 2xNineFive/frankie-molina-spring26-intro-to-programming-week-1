...
// ---------- QUESTION 3 ---------
// Now let's see how we can loop thru the properties - this is a common task in JavaScript. Use a for...in loop, a special type of loop in JavaScript designed specifically for iterating over the properties of an object. See https://www.w3schools.com/jsref/jsref\_forin.asp for reference.
...
const printPets = function (petsArray) {
...
const areObjectsEqual = function (comparison1, comparison2) {
    if (Object.keys(comparison1).length !== Object.keys(comparison2).length) {
        return false;
    } 

    const array1 = (Object.keys(comparison1));
    for (let i = 0; i < array1.length; i++) {
        if (comparison1[array1[i]] !== comparison2[array1[i]]) {
            return false;
        }
    }
    return true;
...
let dog4 = {name : "Yogurt", breed : "Doberman", age : .4, weightInPounds : 20 }
